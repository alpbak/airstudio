...
*/
		...
		flowplayerStandard : function() {
			flowplayer("player", {src: settings.flowPlayerFlash, wmode: "transparent"}, {
				autoPlay	: true,
				onBeforeLoad: function() {
					$('#streifen').animate({'height':settings.videoHeight},settings.speed);
					private.player.animate({'height':settings.videoHeight},settings.speed);
					private.wrap.addClass('play').animate({'height':settings.videoHeight},settings.speed);
		...
		flowplayerIphone : function() {
			flowplayer("player","pub/flash/flowplayer-3.2.7.swf",{
    			...
    			onBeforeLoad: function() {
    				...
    				$('#streifen').animate({'height':settings.videoHeight},settings.speed);
    				private.player.css({'height':settings.videoHeight});
    				private.header.addClass('play').animate({'height':settings.videoHeight},settings.speed);
		...
		backToTheRoots : function() {
			// Elemente, die ebenfalls mitanimiert werden sollen (Selektoren)
			$('#streifen').animate({'height':200},400);
			private.player.animate({'height':200},400);
			private.wrap.animate({'height':200},400).removeClass('play');
	...
	var methods = {
		...
		init : function(options) {
			...
			$(this).find('.vimeo').each(function() {
				$(this).find('.vimeoOverlay').css({ width: '100%', height: '170px' });
		 ...
		 */
		flowplayerStandard : function() {
			flowplayer(private.currObj.attr('id'), {src: settings.flowPlayerFlash, wmode: "transparent"}, {
				autoPlay	: true,
				plugins: {
                    controls: {
                        all: false,
...
(function( $ ){
	...
	var methods = {
		...
		init : function(options) {
			...
			private.li.find('a').css({'width':settings.width,'height':settings.height,'display':'block'});
			...
			private.li.find('span.playWrap').css({'position':'absolute','left':(private.li.width()/2),'margin-left':-(settings.playBtnWidth/2),'top':(settings.height/2),'margin-top':-(settings.playBtnHeight/2),'overflow':'hidden','display':'block','width':settings.playBtnWidth,'height':settings.playBtnHeight});
		 ...
		 */
		flowplayerStandard : function() {
			flowplayer("videoGalleryPlayer"+private.idCount, {src: settings.flowPlayerFlash, wmode: "transparent"}, {
				autoPlay	: true,
				plugins: {
                    controls: {
                        all: false,
